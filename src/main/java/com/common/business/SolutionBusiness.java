...
import com.common.persistence.AbstractSolutionExporter;
import com.common.persistence.AbstractSolutionImporter;
import com.common.swingui.SolverAndPersistenceFrame;
import com.persistence.XStreamTravelingTournamentFileIO;
import com.solver.termination.CustomTermination;
...
public class SolutionBusiness<Solution_, Score_ extends Score<Score_>> {
    ...
    public void openSolution(File file) {
        ...
        XStreamTravelingTournamentFileIO MysolutionFileIO = (XStreamTravelingTournamentFileIO) solutionFileIO;

        Boolean ignoreDaysAssignment = true;
        Solution_ solution = (Solution_) ((XStreamTravelingTournamentFileIO) solutionFileIO).read(file, ignoreDaysAssignment);
        LOGGER.info("Opened: {}", file);
        solutionFileName = file.getName();
        guiScoreDirector.setWorkingSolution(solution);

        Score score = guiScoreDirector.calculateScore();
        CustomTermination.setStartingInitializedScore(score);

        solution = solutionFileIO.read(file);
        guiScoreDirector.setWorkingSolution(solution);
    }
