...
import static com.common.swingui.timetable.TimeTablePanel.HeaderColumnKey.*;
import static com.common.swingui.timetable.TimeTablePanel.HeaderRowKey.*;
...
import com.common.swingui.SolutionPanel;
import com.common.swingui.components.LabeledComboBoxRenderer;
import com.common.swingui.timetable.TimeTablePanel;
...
public class TravelingTournamentPanel extends SolutionPanel<TravelingTournament> {
    ...
    public static final String LOGO_PATH = "/swingui/travelingTournamentLogo.png";
    ...
    private void fillCells(TravelingTournament travelingTournament) {
        ...
        fillBreaks(travelingTournament);
        fillNoHomePeriods(travelingTournament);
    }
    ...
    private void fillMatchCells(TravelingTournament travelingTournament) {
        ...
        for (Match match : travelingTournament.getMatchList()) {
            ...
            if (homeTeam.getMatchType(match.getDay()) != MatchType.AWAY) {
                teamsPanel.addCell(homeTeam, match.getDay(),
                        createButton(match, homeTeam, awayTeam, toolTip));
            }

            if (awayTeam.getMatchType(match.getDay()) != MatchType.AWAY) {
    ...
    private void fillBreaks(TravelingTournament travelingTournament) {
        for (Day day : travelingTournament.getDayList()) {
            if (day.getActiveFlag() == false) {
                for (Team team : travelingTournament.getTeamList()) {

                    JPanel cellComponent = createTableHeader(new JLabel());
                    cellComponent.setBackground(Color.PINK);
                    teamsPanel.addCell(team, day, cellComponent);
                }
            }
        }
    }

    private void fillNoHomePeriods(TravelingTournament travelingTournament) {
        for (Team team : travelingTournament.getTeamList()) {
            for (Day day : travelingTournament.getDayList()) {
                if (team.getMatchType(day) == MatchType.AWAY && day.getActiveFlag() == true) {
                    Match match = travelingTournament.findMatch(team, day);
                    ...
                    if (match == null) {
                        ...
                        String toolTip = determinePlanningEntityTooltip(match);
                        teamsPanel.addCell(team, match.getDay(),
                                createButton2(Color.LIGHT_GRAY, match, team, match.getAnotherTeam(team), toolTip));
                    }
                }
            }
        }
    ...
    private JButton createButton(Match match, Team team, Team otherTeam, String toolTip) {
        ...
        String label = otherTeam.getLabel() + ((match.getFixingState())?"*":"");
    ...
    private JButton createButton2(Color color, Match match, Team team, Team otherTeam, String toolTip) {
        String label = otherTeam.getLabel() + ((match.getFixingState())?"*":"");
        JButton button = SwingUtils.makeSmallButton(new JButton(new MatchAction(match, label)));
        if (match.getAwayTeam() == team) {
            button.setIcon(awayMatchIcon);
