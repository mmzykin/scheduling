...
public class SeriaLengthConstraint extends Constraint {
    ...
    public void insert(Team team, Seria seria) {
         ...
         */
            ...
            if(seria.getSeriaType() == MatchType.AWAY &&
               seriaLength > maxSeriaLengthParam && seriaLength <= excParamMaxSeriaLength && 
               team.checkHomeNaCondition(seria.getFirstMatch().getDay(), seria.getLastMatch().getDay(), excParamMinConsecutiveHomeNa, 1.0) == true) {     
             //       consecutiveHomeNa >= excParamMinConsecutiveHomeNa) {
